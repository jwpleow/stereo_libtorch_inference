...
{
    public:
        StereoRectifier(const std::string& stereo_calib_file);
        ...
        and then crops the images by stereo_calib_params.valid_matched_roi calculated by calculateMatchedRoi()
        */
        void rectify(const cv::Mat &input_left, const cv::Mat &input_right, cv::Mat &output_left, cv::Mat &output_right);
        // splits the image into left and right half (not a deepcopy!)
...
class StereoCamera : public CameraBase, public StereoRectifier
{
    public:
